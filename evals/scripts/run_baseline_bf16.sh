#!/bin/bash
# Run baseline BF16 evaluation using native HF backend
# Evaluates the untrained base model in full precision (bfloat16)
#
# This establishes the pre-training regression baseline to compare
# against the fine-tuned model. Running both in bf16 avoids the
# Q4_K_M quantization penalty that collapses HumanEval from
# 70.73% to 1.83%.
#
...
for arg in "$@"; do
    case "$arg" in
        --help)
            show_help "$0" "Baseline BF16 Evaluation (native HF, full precision)" "$TASKS_LIST"
            exit 0
            ;;
        --dry-run)
            DRY_RUN="true"
...
log_section "Baseline BF16 Evaluation"
