#!/bin/bash
# Run Q4_K_M baseline evaluation (pre-quantized GGUF from unsloth/Qwen3.5-9B-GGUF)
# Isolates quantization penalty from fine-tuning effect
#
...
#
# Usage:
#   ./run_baseline_q4.sh                          # Run all tasks
#   ./run_baseline_q4.sh --tasks humaneval        # Run single task only
#   ./run_baseline_q4.sh --tasks mmlu_pro,humaneval  # Run specific tasks
#   ./run_baseline_q4.sh --help                   # Show available tasks
...
RESULTS_DIR="$PROJECT_DIR/results/baseline/q4"
...
# Model paths (cached GGUF) - NOTE: qwen35 GGUF not supported by transformers yet
# Using safetensors baseline instead
...
for arg in "$@"; do
    case "$arg" in
        --help)
            show_help "$0" "Q4_K_M Baseline Evaluation" "$TASKS_LIST"
            exit 0
            ;;
        --dry-run)
            DRY_RUN="true"
...
log_section "Q4_K_M Baseline Evaluation"
