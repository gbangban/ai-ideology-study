#!/bin/bash
# Run GRPO BF16 evaluation using native HF backend
# Evaluates the GRPO-merged full-precision safetensors model
#
# This uses BF16 to avoid the Q4_K_M quantization penalty, providing
# a fair comparison with baseline and SFT-finetuned evaluations.
#
# NOTE: Requires Studio container to be stopped (GPU must be free)
# NOTE: The full 9B bf16 model uses ~18GB VRAM
#
# Usage:
#   ./run_grpo_bf16.sh                                # Run all tasks
#   ./run_grpo_bf16.sh --tasks humaneval               # Run single task only
#   ./run_grpo_bf16.sh --tasks mmlu_pro,humaneval      # Run specific tasks
#   ./run_grpo_bf16.sh --help                          # Show available tasks
#
# Set GRPO_MODEL_DIR to point to a specific checkpoint directory:
#   GRPO_MODEL_DIR=/path/to/checkpoint ./run_grpo_bf16.sh
...
RESULTS_DIR="$PROJECT_DIR/results/grpo/bf16"
...
if [ ! -d "$MODEL_DIR" ]; then
    log_error "GRPO model directory not found: $MODEL_DIR"
    log_error "Set GRPO_MODEL_DIR env var to override."
    exit 1
fi
...
for arg in "$@"; do
    case "$arg" in
        --help)
            show_help "$0" "GRPO BF16 Evaluation (native HF, full precision merged model)" "$TASKS_LIST"
            exit 0
            ;;
        --dry-run)
            DRY_RUN="true"
...
log_section "GRPO BF16 Evaluation"
